...
export const getCompletionStatus = async (req, res) => {
  try {
    const { data } = await supabase
      ...
      .maybeSingle();
    res.json({ claimed: !!data });
  } catch {
    res.json({ claimed: false });
  }
};

// POST /api/profiles/completion/claim — reclamar 50 coins por perfil 100% completo
export const claimCompletionReward = async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    if (existing) {
      return res.status(409).json({ error: 'Ya reclamaste este premio', code: 'ALREADY_CLAIMED' });
    }

    const [{ data: profile }, { count: photoCount }] = await Promise.all([
      supabase.from('profiles')
        .select('avatar_url, full_name, age, gender, bio, country, language')
        .eq('id', userId).single(),
    ...
    const steps = [
      { done: !!profile.avatar_url, pct: 20 },
      { done: !!profile.full_name,  pct: 10 },
      { done: !!profile.age,        pct: 10 },
      { done: !!profile.bio,        pct: 15 },
      { done: !!profile.gender,     pct: 10 },
      { done: !!profile.country,    pct: 10 },
      { done: !!profile.language,   pct: 10 },
      { done: (photoCount || 0) > 0, pct: 15 },
    ];
    const total = steps.filter(s => s.done).reduce((a, s) => a + s.pct, 0);
    if (total < 100) {
      return res.status(400).json({ error: 'Perfil incompleto', completeness: total, code: 'INCOMPLETE_PROFILE' });
    }

    await addCoins(userId, 50, 'profile_completion');

    res.json({ coins: 50, message: '¡50 coins por completar tu perfil!' });
  } catch (err) {
    res.status(500).json({ error: 'Error interno del servidor' });
  }
};
