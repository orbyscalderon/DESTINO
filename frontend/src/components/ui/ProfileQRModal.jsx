  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="bg-white rounded-2xl p-5 mb-4 flex justify-center" data-no-invert>
          <QRCodeSVG value={url} size={220} level="M" includeMargin={false} />
        </div>

        <div className="flex items-center gap-2 bg-dark-700 rounded-xl p-2 mb-3">
          <span className="text-gray-400 text-xs truncate flex-1 px-2">{url}</span>
          <button onClick={copyLink} className="text-brand-400 hover:text-brand-300 px-2">
            {copied ? <FiCheck size={14} /> : <FiCopy size={14} />}
          </button>
        </div>

        <button onClick={share} className="btn-primary w-full text-sm">
          Compartir enlace
        </button>
      </motion.div>
    </motion.div>
