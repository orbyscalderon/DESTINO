...
export default function MessageLimitBanner({ remaining, limit, onUpgrade }) {
  ...
  if (remaining <= 0) {
    return (
      <div className="bg-brand-500/10 border border-brand-500/30 mx-4 mt-3 rounded-xl p-3 flex items-center justify-between">
        <span className="text-sm text-brand-400">Has alcanzado tu límite diario de mensajes</span>
        <button onClick={onUpgrade} className="text-xs bg-brand-500 text-white px-3 py-1.5 rounded-lg font-semibold flex items-center gap-1 hover:opacity-90">
