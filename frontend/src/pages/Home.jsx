...
export default function Home() {
  ...
  const [lastSwiped, setLastSwiped]   = useState(null); // { profile, action }
  ...
  const loadFeed = async (f = activeFilters) => {
    ...
    try {
      ...
      setFeed(profiles);
      // Preload next card's avatar in background
  ...
  const handleLike = async (targetId) => {
    ...
    try {
      trackAction(); // anuncio cada 5 swipes para usuarios gratuitos
  ...
  return (
    <div className="min-h-screen px-4 pt-8 pb-6 lg:px-10 lg:pt-10">
      ...
      {/* Stories row — siempre visible, con slot propio aunque no haya stories */}
      <div className="flex gap-4 overflow-x-auto pb-3 mb-6 scrollbar-hide -mx-1 px-1">
        {/* Slot "Mi story" — siempre primero */}
        {(() => {
          const ownGroup = storyGroups.find(g => g.user.id === profile?.id);
          if (ownGroup) {
            // El usuario YA tiene stories — abrir el viewer
            return (
              <button
          ...
          // Sin stories propias → botón de añadir
          return (
            <button
      ...
      {/* Profile Preview Modal (long-press) */}
      <AnimatePresence>
        {previewProfile && (
          <motion.div
            initial={{ opacity: 0 }}
